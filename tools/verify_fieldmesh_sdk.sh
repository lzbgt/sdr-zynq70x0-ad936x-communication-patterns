...
"$repo_root/tools/verify_fieldmesh_z103_management_defaults.sh"
"$repo_root/tools/verify_fieldmesh_z103_recovery_state.sh"
"$repo_root/tools/verify_fieldmesh_sidecar_preflight.sh"
"$repo_root/tools/verify_fieldmesh_board_dma_smoke_guard.sh"
"$repo_root/tools/verify_fieldmesh_board_fw_dma_control.sh"
"$repo_root/tools/verify_fieldmesh_board_rf_phy_bind_gate.sh"
"$repo_root/tools/verify_fieldmesh_rf_tx_guard_apply.sh"
"$repo_root/tools/verify_fieldmesh_rf_source_apply.sh"
"$repo_root/tools/verify_fieldmesh_rf_hardware_progression_evidence.sh"
"$repo_root/tools/verify_fieldmesh_fw_dma_control_contract.sh"
"$repo_root/tools/verify_fieldmesh_sidecar_addr_contract.sh"
"$repo_root/tools/verify_fieldmesh_runtime_source_freshness.sh"
"$repo_root/tools/verify_fieldmesh_yocto_builder_contract.sh"
"$repo_root/tools/verify_fieldmesh_rf_engine_firmware_dma_binding.sh"
"$repo_root/tools/verify_fieldmesh_rf_tx_enable_plan.sh"
"$repo_root/tools/verify_fieldmesh_rf_tx_enable_run.sh"
...
"$repo_root/tools/verify_fieldmesh_gnss_live_preflight_summary.sh"
"$repo_root/tools/verify_fieldmesh_gnss_timepulse_plan.sh"
"$repo_root/tools/verify_fieldmesh_gnss_timepulse_poll.sh"
"$repo_root/tools/verify_fieldmesh_gnss_timepulse_apply.sh"
"$repo_root/tools/verify_fieldmesh_gnss_pps_diagnostic_sequence.sh"
"$repo_root/tools/verify_fieldmesh_gnss_devicetree_binding.sh"
"$repo_root/tools/verify_fieldmesh_gnss_uart_emio_overlay.sh"
"$repo_root/tools/verify_fieldmesh_gnss_pps_emio_overlay.sh"
"$repo_root/tools/verify_fieldmesh_iio_preflight_assert.sh"
"$repo_root/tools/verify_fieldmesh_iio_burst_xfer.sh"
"$repo_root/tools/verify_fieldmesh_iio_rf_worker_bridge_loop.sh"
"$repo_root/tools/verify_fieldmesh_rf_path_evidence_author.sh"
"$repo_root/tools/verify_fieldmesh_native_ip_feature_readiness.sh"
"$repo_root/tools/verify_fieldmesh_native_ip_iperf_production_sequence.sh"
"$repo_root/tools/verify_fieldmesh_conducted_rf_preflight.sh"
"$repo_root/tools/verify_fieldmesh_conducted_rf_production_sequence.sh"
"$repo_root/tools/verify_fieldmesh_conducted_rf_evidence_manifest.sh"
...
rf_phy_bind_validate = [row for row in query if row.get("event") == "sdk_daemon_rf_phy_driver_bind_validate"]
rf_phy_bind_apply = [row for row in query if row.get("event") == "sdk_daemon_rf_phy_driver_bind_apply"]
...
if hello[0].get("native_client_ip_mode") != "routed_l3_swarm0":
    raise SystemExit("SDK daemon native IP mode changed")
if hello[0].get("native_client_ip_interface") != "swarm0":
    raise SystemExit("SDK daemon native IP interface changed")
...
if not app_message or app_message[0].get("ok") is not True:
    raise SystemExit("SDK daemon app message send path failed")
if app_message[0].get("queued_to_rf_engine") != 1:
    raise SystemExit("SDK daemon app message was not queued to RF packet engine")
if app_message[0].get("uses_json_on_air") != 0:
    raise SystemExit("SDK daemon app messages must not use JSON on air")
...
if not tun_fd_pump_burst or tun_fd_pump_burst[0].get("adapter_name") != "swarm0":
    raise SystemExit("SDK daemon TUN fd pump burst query failed")
if tun_fd_pump_burst[0].get("fd_source") != "event_callback_batch":
    raise SystemExit("SDK daemon TUN burst pump did not use event callback source")
if tun_fd_pump_burst[0].get("dst_device_eui") != "020000000103":
    raise SystemExit("SDK daemon TUN burst pump used wrong destination EUI")
if tun_fd_pump_burst[0].get("packets_read") != 3 or tun_fd_pump_burst[0].get("packets_sent") != 3:
    raise SystemExit("SDK daemon TUN burst pump packet counts failed")
if tun_fd_pump_burst[0].get("packets_rx_loopback") != 3:
    raise SystemExit("SDK daemon TUN burst pump did not loop back all packets")
if tun_fd_pump_burst[0].get("event_loop_ready") != 1 or tun_fd_pump_burst[0].get("bounded_batch") != 1:
    raise SystemExit("SDK daemon TUN burst pump event-loop metadata failed")
if tun_fd_pump_burst[0].get("sent_to_fieldmesh_adapter") != 1:
    raise SystemExit("SDK daemon TUN burst pump did not reach the adapter")
if tun_fd_pump_burst[0].get("next_boundary") != "fieldmesh_rf_packet_engine":
    raise SystemExit("SDK daemon TUN burst pump next boundary is wrong")
for key in ("uses_iio", "uses_inter_board_ip_routing"):
    if tun_fd_pump_burst[0].get(key) != 0:
        raise SystemExit(f"SDK daemon TUN burst pump key {key} must be 0")
...
if not tun_device_drain_guard or tun_device_drain_guard[0].get("adapter_name") != "swarm0":
    raise SystemExit("SDK daemon TUN device drain guard query failed")
if tun_device_drain_guard[0].get("production_tun_path") != "/dev/net/tun":
    raise SystemExit("SDK daemon TUN device drain guard lost production TUN path")
for key in ("requires_allow_live_tun_write", "requires_cap_net_admin", "requires_existing_swarm0"):
    if tun_device_drain_guard[0].get(key) != 1:
        raise SystemExit(f"SDK daemon TUN device drain guard key {key} must be 1")
for key in ("opens_dev_net_tun", "attaches_tun_if", "writes_to_tun", "commands_executed",
            "writes_network", "uses_iio", "uses_inter_board_ip_routing"):
    if tun_device_drain_guard[0].get(key) != 0:
        raise SystemExit(f"SDK daemon TUN device drain guard key {key} must be 0")
if tun_device_drain_guard[0].get("next_boundary") != "client_kernel_ip_stack":
    raise SystemExit("SDK daemon TUN device drain guard next boundary is wrong")
if not tun_event_loop_guard or tun_event_loop_guard[0].get("adapter_name") != "swarm0":
    raise SystemExit("SDK daemon TUN event-loop guard query failed")
if tun_event_loop_guard[0].get("production_tun_path") != "/dev/net/tun":
    raise SystemExit("SDK daemon TUN event-loop guard lost production TUN path")
for key in ("requires_allow_live_tun_read", "requires_allow_live_tun_write",
    ...
    if tun_event_loop_guard[0].get(key) != 1:
        raise SystemExit(f"SDK daemon TUN event-loop guard key {key} must be 1")
...
if tun_service_irq_ack_guard[0].get("ok") is not False:
    raise SystemExit("firmware ring IRQ ACK must be guarded by default")
if tun_service_irq_ack_guard[0].get("error") != "firmware_ring_irq_ack_requires_guard":
    raise SystemExit("firmware ring IRQ ACK guard error changed")
if tun_service_irq_ack_guard[0].get("requires_allow_firmware_ring_writes") != 1:
    raise SystemExit("firmware ring IRQ ACK guard must require write opt-in")
if tun_service_irq_ack_guard[0].get("firmware_ring_supported") != 1:
    raise SystemExit("firmware ring IRQ ACK guard must expose firmware ring support")
for key in ("writes_hardware", "uses_iio", "uses_json_on_air"):
    if tun_service_irq_ack_guard[0].get(key) != 0:
...
if rf_phy_bind_validate[0].get("driver") != "fieldmesh_rf_packet_engine":
    raise SystemExit("SDK daemon RF PHY bind validation driver changed")
if rf_phy_bind_validate[0].get("adapter_name") != "swarm0":
...
for key in ("requires_sidecar_preflight", "requires_sidecar_dma",
    ...
    if rf_phy_bind_validate[0].get(key) != 1:
        raise SystemExit(f"SDK daemon RF PHY bind validation key {key} must be 1")
for key in ("conducted_or_shielded", "legal_frequency_profile", "rx_first",
            "measured_link", "prerequisites_ready", "binding_ready",
            "live_rf_allowed", "rf_phy_tx_rx", "rf_phy_tx_rx_verified",
            "app_verified_real_rf", "production_ready", "opens_iio_buffers",
            "starts_rf_tx", "writes_hardware", "commands_executed",
            "uses_inter_board_ip_routing", "uses_json_on_air"):
    if rf_phy_bind_validate[0].get(key) != 0:
        raise SystemExit(f"SDK daemon RF PHY bind validation key {key} must be 0")
if rf_phy_bind_validate[0].get("sidecar_preflight_passed") != 1:
    raise SystemExit("SDK daemon RF PHY bind validation did not preserve sidecar preflight evidence flag")
if rf_phy_bind_validate[0].get("sidecar_dma_passed") != 1:
    raise SystemExit("SDK daemon RF PHY bind validation did not preserve sidecar DMA evidence flag")
if rf_phy_bind_validate[0].get("rf_packet_engine_passed") != 1:
...
for key in ("live_rf_allowed", "rf_phy_tx_rx", "rf_phy_tx_rx_verified",
            ...
            "starts_rf_tx", "writes_hardware", "commands_executed",
            "uses_inter_board_ip_routing", "uses_json_on_air"):
    if rf_phy_bind_apply[0].get(key) != 0:
        raise SystemExit(f"SDK daemon RF PHY bind apply key {key} must be 0")
...
drain = [event for event in events if event.get("event") == "sdk_tun_packetizer_drain_many"]
if not opened or opened[0].get("adapter_name") != "swarm0":
...
if not drain:
    raise SystemExit("TUN packetizer did not verify adapter-to-TUN drain path")
if drain[0].get("adapter_name") != "swarm0":
    raise SystemExit("TUN packetizer drain did not target swarm0")
if drain[0].get("dst_device_eui") != "020000000103":
    raise SystemExit("TUN packetizer drain did not preserve compact EUI")
if drain[0].get("packets_received") != 3 or drain[0].get("packets_written") != 3:
    raise SystemExit("TUN packetizer drain packet counts failed")
if drain[0].get("written_to_tun") != 1 or drain[0].get("received_from_fieldmesh_adapter") != 1:
    raise SystemExit("TUN packetizer drain did not bridge adapter to TUN")
for key in ("uses_iio", "uses_inter_board_ip_routing"):
    if drain[0].get(key) != 0:
        raise SystemExit(f"TUN packetizer drain key {key} must be 0")
if drain[0].get("next_boundary") != "client_kernel_ip_stack":
    raise SystemExit("TUN packetizer drain next boundary is wrong")
PY
