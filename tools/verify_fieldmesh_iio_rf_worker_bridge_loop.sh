...
if captured_native_tick.get("text") != (
    ...
    "current_consecutive_direction_batches=0 "
    "primary_samples_per_symbol=8 primary_bit_repeat=1 "
...
try:
    iio_execute = loop.iio_transport_daemon_execute(
        "127.0.0.1", 55441, 10, 2, 384, 8, 1
    )
finally:
    bridge.request_daemon = original_request
if captured.get("text") != (
    "FIELDMESH_IIO_TRANSPORT_DAEMON_EXECUTE v1 "
    "frames=2 bytes=384 samples_per_symbol=8 bit_repeat=1"
):
    raise SystemExit(
        f"state-daemon IIO transport execute must use daemon execute command: {captured}"
    )
loop.validate_iio_transport_daemon_execute(iio_execute, "z203-to-z103", 2, 384, 8, 1)
