...
policy = next((row for row in apply if row.get("event") == "rf_source_apply_policy"), None)
...
if policy.get("writes_registers") is not False:
    raise SystemExit(f"RF source action policy check must be read-only: {policy}")
...
if write.get("readback_ok") is not True:
    raise SystemExit(f"RF source apply did not verify source-select readback: {write}")
...
python3 - "$repo_root/tools/run_fieldmesh_board_rf_source_apply.sh" <<'PY'
import sys
from pathlib import Path
...
required = [
    ...
    "Board fieldmesh-udp-probe lacks current rf-source apply/action-policy self-test contract",
]
for token in required:
    if token not in script:
        raise SystemExit(f"board RF source wrapper missing self-test token: {token}")
...
apply = script.index("rf-source-apply$(shell_words")
summary = script.index('"event": "fieldmesh_board_rf_source_apply_assert"')
if upload_check > self_test_run:
    raise SystemExit("board RF source wrapper must check self-test freshness before running it")
if self_test_run > scan_before:
    raise SystemExit("board RF source wrapper must run C self-test before RF guard scan evidence")
if script.index("expected_self") > apply:
    raise SystemExit("board RF source wrapper must validate C self-test before source apply")
if script.index("rf_guard_action_policy_self_test_ok") < summary:
    raise SystemExit("board RF source wrapper must summarize validated self-test proof")
PY
