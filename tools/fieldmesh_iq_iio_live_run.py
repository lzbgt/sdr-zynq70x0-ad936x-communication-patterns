...
class BurstHelperServer:
    def __init__(self, argv: list[str], timeout_s: float) -> None:
        ...
        self.transport_autonomous_loop_start: dict[str, Any] = {}
        self.transport_autonomous_loop_status: dict[str, Any] = {}
        self.transport_background_daemon_start: dict[str, Any] = {}
        self.transport_integrated_rf_service_daemon_start: dict[str, Any] = {}
        self.transport_integrated_rf_service_daemon_status: dict[str, Any] = {}
    ...
    def transport_autonomous_loop_run(self, queue_file: Path, timeout_s: float) -> dict[str, Any]:
        if self.proc.stdin is None:
            raise SystemExit("iio_burst_helper server stdin is unavailable")
        if self.proc.poll() is not None:
            stderr = self._stderr_after_exit().strip()
            raise SystemExit(f"iio_burst_helper server is not running: stderr={stderr}")
        line = f"TRANSPORT_AUTONOMOUS_LOOP_RUN queue_file={queue_file}\n"
        try:
            self.proc.stdin.write(line)
            self.proc.stdin.flush()
        except BrokenPipeError as exc:
            stderr = self._stderr_after_exit().strip()
            raise SystemExit(f"iio_burst_helper server pipe broke: stderr={stderr}") from exc
        return self._read_json_line(timeout_s, "transport_autonomous_loop_run")

    def start_transport_background_daemon(self, queue_file: Path, timeout_s: float) -> dict[str, Any]:
        if self.proc.stdin is None:
            raise SystemExit("iio_burst_helper server stdin is unavailable")
        if self.proc.poll() is not None:
            stderr = self._stderr_after_exit().strip()
            raise SystemExit(f"iio_burst_helper server is not running: stderr={stderr}")
        line = f"TRANSPORT_BACKGROUND_DAEMON_START queue_file={queue_file}\n"
        try:
            self.proc.stdin.write(line)
            self.proc.stdin.flush()
        except BrokenPipeError as exc:
            stderr = self._stderr_after_exit().strip()
            raise SystemExit(f"iio_burst_helper server pipe broke: stderr={stderr}") from exc
        report = self._read_json_line(timeout_s, "transport_background_daemon_start")
        if (
            report.get("event") != "fieldmesh_iio_burst_transport_background_daemon_start"
            or report.get("ok") is not True
            or report.get("native_iio_burst_transport_background_daemon_proof")
            != "FIELDMESH_IIO_BURST_NATIVE_TRANSPORT_BACKGROUND_DAEMON v1"
        ):
            self.close(kill=True)
            raise SystemExit(f"iio_burst_helper transport background daemon did not start: {report}")
        self.transport_background_daemon_start = report
        return report

    def start_transport_integrated_rf_service_daemon(self, timeout_s: float) -> dict[str, Any]:
        if self.proc.stdin is None:
            raise SystemExit("iio_burst_helper server stdin is unavailable")
        if self.proc.poll() is not None:
            stderr = self._stderr_after_exit().strip()
            raise SystemExit(f"iio_burst_helper server is not running: stderr={stderr}")
        if self.transport_integrated_rf_service_daemon_start:
            return self.transport_integrated_rf_service_daemon_start
        try:
            self.proc.stdin.write("TRANSPORT_INTEGRATED_RF_SERVICE_DAEMON_START\n")
            self.proc.stdin.flush()
        except BrokenPipeError as exc:
            stderr = self._stderr_after_exit().strip()
            raise SystemExit(f"iio_burst_helper server pipe broke: stderr={stderr}") from exc
        report = self._read_json_line(timeout_s, "transport_integrated_rf_service_daemon_start")
        if (
            report.get("event") != "fieldmesh_iio_burst_integrated_rf_service_daemon_start"
            or report.get("ok") is not True
            or report.get("native_iio_burst_integrated_rf_service_daemon_proof")
            != "FIELDMESH_IIO_BURST_INTEGRATED_RF_SERVICE_DAEMON v1"
        ):
            self.close(kill=True)
            raise SystemExit(f"iio_burst_helper integrated RF service daemon did not start: {report}")
        self.transport_integrated_rf_service_daemon_start = report
        return report

    def enqueue_transport_integrated_rf_service_daemon(
        self, queue_file: Path, timeout_s: float
    ) -> dict[str, Any]:
        if self.proc.stdin is None:
            raise SystemExit("iio_burst_helper server stdin is unavailable")
        if self.proc.poll() is not None:
            stderr = self._stderr_after_exit().strip()
            raise SystemExit(f"iio_burst_helper server is not running: stderr={stderr}")
        line = f"TRANSPORT_INTEGRATED_RF_SERVICE_DAEMON_ENQUEUE queue_file={queue_file}\n"
        try:
            self.proc.stdin.write(line)
            self.proc.stdin.flush()
        except BrokenPipeError as exc:
            stderr = self._stderr_after_exit().strip()
            raise SystemExit(f"iio_burst_helper server pipe broke: stderr={stderr}") from exc
        report = self._read_json_line(timeout_s, "transport_integrated_rf_service_daemon_enqueue")
        if (
            report.get("event") != "fieldmesh_iio_burst_integrated_rf_service_daemon_enqueue"
            or report.get("ok") is not True
            or report.get("native_iio_burst_integrated_rf_service_daemon_proof")
            != "FIELDMESH_IIO_BURST_INTEGRATED_RF_SERVICE_DAEMON v1"
        ):
    ...
    def status_transport_integrated_rf_service_daemon(self, timeout_s: float) -> dict[str, Any]:
        ...
        if self.proc.poll() is not None:
            ...
            raise SystemExit(f"iio_burst_helper server is not running: stderr={stderr}")
        try:
            self.proc.stdin.write("TRANSPORT_INTEGRATED_RF_SERVICE_DAEMON_STATUS\n")
            self.proc.stdin.flush()
        except BrokenPipeError as exc:
            stderr = self._stderr_after_exit().strip()
            raise SystemExit(f"iio_burst_helper server pipe broke: stderr={stderr}") from exc
        report = self._read_json_line(timeout_s, "transport_integrated_rf_service_daemon_status")
        if report.get("event") != "fieldmesh_iio_burst_integrated_rf_service_daemon_status":
            self.close(kill=True)
            raise SystemExit(f"iio_burst_helper integrated RF service daemon status invalid: {report}")
        self.transport_integrated_rf_service_daemon_status = report
        return report

    def wait_transport_background_daemon(self, timeout_s: float) -> tuple[dict[str, Any], dict[str, Any]]:
        if self.proc.stdin is None:
            raise SystemExit("iio_burst_helper server stdin is unavailable")
        deadline = time.monotonic() + timeout_s
        ...
        while True:
            ...
            if remaining <= 0:
                self.close(kill=True)
                raise SystemExit(
                    f"iio_burst_helper background daemon timed out: {last_status}"
                )
            if self.proc.poll() is not None:
                stderr = self._stderr_after_exit().strip()
                raise SystemExit(f"iio_burst_helper server is not running: stderr={stderr}")
            try:
                self.proc.stdin.write("TRANSPORT_BACKGROUND_DAEMON_STATUS\n")
                self.proc.stdin.flush()
            except BrokenPipeError as exc:
                stderr = self._stderr_after_exit().strip()
                raise SystemExit(f"iio_burst_helper server pipe broke: stderr={stderr}") from exc
            report = self._read_json_line(min(max(remaining, 0.1), 1.0), "transport_background_daemon_status")
            if report.get("event") == "fieldmesh_iio_burst_xfer":
                return report, last_status
            if report.get("event") != "fieldmesh_iio_burst_transport_background_daemon_status":
                self.close(kill=True)
                raise SystemExit(f"iio_burst_helper background daemon status invalid: {report}")
            last_status = report
            time.sleep(min(0.05, max(deadline - time.monotonic(), 0.0)))

    def status_transport_background_daemon(self, timeout_s: float) -> dict[str, Any]:
        if self.proc.stdin is None:
            raise SystemExit("iio_burst_helper server stdin is unavailable")
        if self.proc.poll() is not None:
            stderr = self._stderr_after_exit().strip()
        ...
        try:
            ...
            stderr = self._stderr_after_exit().strip()
            raise SystemExit(f"iio_burst_helper server pipe broke: stderr={stderr}") from exc
        report = self._read_json_line(timeout_s, "transport_background_daemon_status")
        if report.get("event") != "fieldmesh_iio_burst_transport_background_daemon_status":
            self.close(kill=True)
            raise SystemExit(f"iio_burst_helper background daemon status invalid: {report}")
        return report
    ...
    def start_transport_autonomous_loop(self, timeout_s: float) -> dict[str, Any]:
        if self.proc.stdin is None:
            raise SystemExit("iio_burst_helper server stdin is unavailable")
        if self.proc.poll() is not None:
            stderr = self._stderr_after_exit().strip()
            raise SystemExit(f"iio_burst_helper server is not running: stderr={stderr}")
        if self.transport_autonomous_loop_start:
            return self.transport_autonomous_loop_start
        try:
            self.proc.stdin.write("TRANSPORT_AUTONOMOUS_LOOP_START\n")
            self.proc.stdin.flush()
        except BrokenPipeError as exc:
            stderr = self._stderr_after_exit().strip()
            raise SystemExit(f"iio_burst_helper server pipe broke: stderr={stderr}") from exc
        report = self._read_json_line(timeout_s, "transport_autonomous_loop_start")
        if (
            report.get("event") != "fieldmesh_iio_burst_transport_autonomous_loop_start"
            or report.get("ok") is not True
            or report.get("native_iio_burst_transport_autonomous_loop_proof")
            != "FIELDMESH_IIO_BURST_NATIVE_TRANSPORT_AUTONOMOUS_LOOP v1"
        ):
            self.close(kill=True)
            raise SystemExit(f"iio_burst_helper transport autonomous loop did not start: {report}")
        self.transport_autonomous_loop_start = report
        return report
    ...
    def status_transport_autonomous_loop(self, timeout_s: float) -> dict[str, Any]:
        if self.proc.stdin is None:
            raise SystemExit("iio_burst_helper server stdin is unavailable")
        if self.proc.poll() is not None:
            stderr = self._stderr_after_exit().strip()
            raise SystemExit(f"iio_burst_helper server is not running: stderr={stderr}")
        try:
            self.proc.stdin.write("TRANSPORT_AUTONOMOUS_LOOP_STATUS\n")
            self.proc.stdin.flush()
        except BrokenPipeError as exc:
            stderr = self._stderr_after_exit().strip()
            raise SystemExit(f"iio_burst_helper server pipe broke: stderr={stderr}") from exc
        report = self._read_json_line(timeout_s, "transport_autonomous_loop_status")
        if report.get("event") != "fieldmesh_iio_burst_transport_autonomous_loop_status":
            self.close(kill=True)
            raise SystemExit(f"iio_burst_helper transport autonomous loop status invalid: {report}")
        self.transport_autonomous_loop_status = report
        return report
...
def execute_live_with_helper(
    ...
    helper_transport_autonomous_loop_start: dict[str, Any] = {}
    helper_transport_autonomous_loop_status: dict[str, Any] = {}
    helper_transport_background_daemon_start: dict[str, Any] = {}
    helper_transport_background_daemon_status: dict[str, Any] = {}
    helper_transport_integrated_rf_service_daemon_start: dict[str, Any] = {}
    helper_transport_integrated_rf_service_daemon_enqueue: dict[str, Any] = {}
    helper_transport_integrated_rf_service_daemon_status: dict[str, Any] = {}
    if getattr(args, "persistent_burst_helper", False):
        ...
        xfer_fields = {
            ...
            "state_daemon_transport_modem_profile": "1",
            "adaptive_mcs_pre_burst_profile_application_source": "state_daemon_iio_transport_lifecycle",
            "state_daemon_transport_selected_samples_per_symbol": str(
                plan["iq_burst"]["samples_per_symbol"]
            ),
            "state_daemon_transport_selected_bit_repeat": str(
                plan["iq_burst"]["bit_repeat"]
            ),
        }
    ...
    helper_result = {
        ...
        "native_iio_burst_transport_autonomous_loop": (
            helper_report.get("native_iio_burst_transport_autonomous_loop") is True
        ),
        "native_iio_burst_transport_autonomous_loop_proof": helper_report.get(
            "native_iio_burst_transport_autonomous_loop_proof"
        ),
        "transport_autonomous_loop_start_count": helper_report.get(
            "transport_autonomous_loop_start_count"
        ),
        "transport_autonomous_loop_start_event": helper_transport_autonomous_loop_start.get("event"),
        "transport_autonomous_loop_start_proof": helper_transport_autonomous_loop_start.get(
            "native_iio_burst_transport_autonomous_loop_proof"
        ),
        "transport_autonomous_loop_status_event": helper_transport_autonomous_loop_status.get("event"),
        "transport_autonomous_loop_status_proof": helper_transport_autonomous_loop_status.get(
            "native_iio_burst_transport_autonomous_loop_proof"
        ),
        "transport_autonomous_loop_status_started": (
            helper_transport_autonomous_loop_status.get(
                "native_iio_burst_transport_autonomous_loop"
            )
            is True
        ),
        "transport_autonomous_loop_run": helper_report.get("transport_autonomous_loop_run") is True,
        "transport_autonomous_loop_run_count": helper_report.get("transport_autonomous_loop_run_count"),
        "transport_autonomous_loop_scheduled_request_count": helper_report.get(
            "transport_autonomous_loop_scheduled_request_count"
        ),
        "native_iio_burst_transport_background_daemon": (
            helper_report.get("native_iio_burst_transport_background_daemon") is True
        ),
        "native_iio_burst_transport_background_daemon_proof": helper_report.get(
            "native_iio_burst_transport_background_daemon_proof"
        ),
        "transport_background_daemon_start_event": helper_transport_background_daemon_start.get(
            "event"
        ),
        "transport_background_daemon_start_proof": (
            helper_transport_background_daemon_start.get(
                "native_iio_burst_transport_background_daemon_proof"
            )
        ),
        "transport_background_daemon_status_event": helper_transport_background_daemon_status.get(
            "event"
        ),
        "transport_background_daemon_status_proof": (
            helper_transport_background_daemon_status.get(
                "native_iio_burst_transport_background_daemon_proof"
            )
        ),
        "transport_background_daemon_status_started": (
            helper_transport_background_daemon_status.get(
                "native_iio_burst_transport_background_daemon"
            )
            is True
        ),
        "transport_background_daemon_start_count": helper_report.get(
            "transport_background_daemon_start_count"
        ),
        "transport_background_daemon_xfer": (
            helper_report.get("transport_background_daemon_xfer") is True
        ),
        "transport_background_daemon_xfer_count": helper_report.get(
            "transport_background_daemon_xfer_count"
        ),
        "transport_background_daemon_scheduled_request_count": helper_report.get(
            "transport_background_daemon_scheduled_request_count"
        ),
        "native_iio_burst_integrated_rf_service_daemon": (
            helper_report.get("native_iio_burst_integrated_rf_service_daemon") is True
        ),
        "native_iio_burst_integrated_rf_service_daemon_proof": helper_report.get(
            "native_iio_burst_integrated_rf_service_daemon_proof"
        ),
        "transport_integrated_rf_service_daemon_start_proof": (
            helper_transport_integrated_rf_service_daemon_start.get(
                "native_iio_burst_integrated_rf_service_daemon_proof"
            )
        ),
        "transport_integrated_rf_service_daemon_enqueue_proof": (
            helper_transport_integrated_rf_service_daemon_enqueue.get(
                "native_iio_burst_integrated_rf_service_daemon_proof"
            )
        ),
        "transport_integrated_rf_service_daemon_status_proof": (
            helper_transport_integrated_rf_service_daemon_status.get(
                "native_iio_burst_integrated_rf_service_daemon_proof"
            )
        ),
        "transport_integrated_rf_service_daemon_status_started": (
            helper_transport_integrated_rf_service_daemon_status.get(
                "native_iio_burst_integrated_rf_service_daemon"
            )
            is True
        ),
        "transport_integrated_rf_service_daemon_start_count": helper_report.get(
            "transport_integrated_rf_service_daemon_start_count"
        ),
        "transport_integrated_rf_service_daemon_enqueue_count": helper_report.get(
        ...
        "native_iio_burst_state_daemon_transport_modem_profile": (
            helper_report.get("native_iio_burst_state_daemon_transport_modem_profile")
            is True
        ),
        "native_iio_burst_state_daemon_transport_modem_profile_proof": (
            helper_report.get(
                "native_iio_burst_state_daemon_transport_modem_profile_proof"
            )
        ),
        "state_daemon_transport_modem_profile_request": (
            helper_report.get("state_daemon_transport_modem_profile_request") is True
        ),
        ...
        "state_daemon_transport_selected_samples_per_symbol": helper_report.get(
            "state_daemon_transport_selected_samples_per_symbol"
        ),
        "state_daemon_transport_selected_bit_repeat": helper_report.get(
        ...
        "python_selected_modem_profile_fields": (
            helper_report.get("python_selected_modem_profile_fields") is True
        ),
        ...
        "python_scheduler_drain_submission": (
            helper_report.get("python_scheduler_drain_submission") is True
        ),
        "python_autonomous_loop_run_submission": (
            helper_report.get("python_autonomous_loop_run_submission") is True
        ),
    ...
    results.append(
        {
            ...
            "native_iio_burst_transport_autonomous_loop": helper_result[
                "native_iio_burst_transport_autonomous_loop"
            ],
            "native_iio_burst_transport_autonomous_loop_proof": helper_result[
                "native_iio_burst_transport_autonomous_loop_proof"
            ],
            "transport_autonomous_loop_start_count": helper_result[
                "transport_autonomous_loop_start_count"
            ],
            "transport_autonomous_loop_start_proof": helper_result[
                "transport_autonomous_loop_start_proof"
            ],
            "transport_autonomous_loop_status_proof": helper_result[
                "transport_autonomous_loop_status_proof"
            ],
            "transport_autonomous_loop_status_started": helper_result[
                "transport_autonomous_loop_status_started"
            ],
            "transport_autonomous_loop_run": helper_result["transport_autonomous_loop_run"],
            "transport_autonomous_loop_run_count": helper_result[
                "transport_autonomous_loop_run_count"
            ],
            "transport_autonomous_loop_scheduled_request_count": helper_result[
                "transport_autonomous_loop_scheduled_request_count"
            ],
            "native_iio_burst_transport_background_daemon": helper_result[
                "native_iio_burst_transport_background_daemon"
            ],
            "native_iio_burst_transport_background_daemon_proof": helper_result[
                "native_iio_burst_transport_background_daemon_proof"
            ],
            "transport_background_daemon_start_proof": helper_result[
                "transport_background_daemon_start_proof"
            ],
            "transport_background_daemon_status_proof": helper_result[
                "transport_background_daemon_status_proof"
            ],
            "transport_background_daemon_status_started": helper_result[
                "transport_background_daemon_status_started"
            ],
            "transport_background_daemon_start_count": helper_result[
                "transport_background_daemon_start_count"
            ],
            "transport_background_daemon_xfer": helper_result["transport_background_daemon_xfer"],
            "transport_background_daemon_xfer_count": helper_result[
                "transport_background_daemon_xfer_count"
            ],
            "transport_background_daemon_scheduled_request_count": helper_result[
                "transport_background_daemon_scheduled_request_count"
            ],
            "native_iio_burst_integrated_rf_service_daemon": helper_result[
                "native_iio_burst_integrated_rf_service_daemon"
            ],
            "native_iio_burst_integrated_rf_service_daemon_proof": helper_result[
                "native_iio_burst_integrated_rf_service_daemon_proof"
            ],
            "transport_integrated_rf_service_daemon_start_proof": helper_result[
                "transport_integrated_rf_service_daemon_start_proof"
            ],
            "transport_integrated_rf_service_daemon_enqueue_proof": helper_result[
                "transport_integrated_rf_service_daemon_enqueue_proof"
            ],
            "transport_integrated_rf_service_daemon_status_proof": helper_result[
                "transport_integrated_rf_service_daemon_status_proof"
            ],
            "transport_integrated_rf_service_daemon_status_started": helper_result[
                "transport_integrated_rf_service_daemon_status_started"
            ],
            "transport_integrated_rf_service_daemon_start_count": helper_result[
                "transport_integrated_rf_service_daemon_start_count"
            ],
            "transport_integrated_rf_service_daemon_enqueue_count": helper_result[
                "transport_integrated_rf_service_daemon_enqueue_count"
            ],
            ...
            "native_iio_burst_state_daemon_transport_modem_profile": helper_result[
                "native_iio_burst_state_daemon_transport_modem_profile"
            ],
            "native_iio_burst_state_daemon_transport_modem_profile_proof": helper_result[
                "native_iio_burst_state_daemon_transport_modem_profile_proof"
            ],
            "state_daemon_transport_modem_profile_request": helper_result[
                "state_daemon_transport_modem_profile_request"
            ],
            ...
            "state_daemon_transport_selected_samples_per_symbol": helper_result[
                "state_daemon_transport_selected_samples_per_symbol"
            ],
            "state_daemon_transport_selected_bit_repeat": helper_result[
                "state_daemon_transport_selected_bit_repeat"
            ],
            "python_iio_helper_modem_profile_mapping": helper_result[
                "python_iio_helper_modem_profile_mapping"
            ],
            "python_selected_modem_profile_fields": helper_result[
                "python_selected_modem_profile_fields"
            ],
            ...
            "python_scheduler_drain_submission": helper_result["python_scheduler_drain_submission"],
            "python_autonomous_loop_run_submission": helper_result[
                "python_autonomous_loop_run_submission"
            ],
            "python_background_daemon_start_submission": helper_result[
                "python_background_daemon_start_submission"
            ],
    ...
    native_iio_burst_transport_scheduler_proven = bool(
        ...
        or (
            ...
            and any(
                ...
                and result.get("python_direct_service_loop_run") is False
                and result.get("python_scheduler_drain_submission") is False
                and result.get("next_boundary") == "native_transport_worker_autonomous_daemon"
                for result in command_results
            )
        )
    ...
    native_iio_burst_transport_autonomous_loop_proven = bool(
        ...
        or (
            ...
            and any(
                result.get("name") == "iio_burst_helper"
                and result.get("returncode") == 0
                and result.get("native_iio_burst_transport_autonomous_loop") is True
                and result.get("native_iio_burst_transport_autonomous_loop_proof")
                == "FIELDMESH_IIO_BURST_NATIVE_TRANSPORT_AUTONOMOUS_LOOP v1"
                and result.get("transport_autonomous_loop_start_proof")
                == "FIELDMESH_IIO_BURST_NATIVE_TRANSPORT_AUTONOMOUS_LOOP v1"
                and result.get("transport_autonomous_loop_status_proof")
                == "FIELDMESH_IIO_BURST_NATIVE_TRANSPORT_AUTONOMOUS_LOOP v1"
                and result.get("transport_autonomous_loop_status_started") is True
                and result.get("transport_autonomous_loop_run") is True
                and isinstance(result.get("transport_autonomous_loop_start_count"), int)
                and result.get("transport_autonomous_loop_start_count") >= 1
                and isinstance(result.get("transport_autonomous_loop_run_count"), int)
                and result.get("transport_autonomous_loop_run_count") >= 1
                and isinstance(
                    result.get("transport_autonomous_loop_scheduled_request_count"), int
                )
                and result.get("transport_autonomous_loop_scheduled_request_count") >= 1
                and result.get("python_direct_service_loop_run") is False
                and result.get("python_scheduler_drain_submission") is False
                and result.get("next_boundary") == "native_transport_worker_autonomous_daemon"
                for result in command_results
    ...
    native_iio_burst_transport_background_daemon_proven = bool(
        ...
        or (
            ...
            and any(
                result.get("name") == "iio_burst_helper"
                and result.get("returncode") == 0
                and result.get("native_iio_burst_transport_background_daemon") is True
                and result.get("native_iio_burst_transport_background_daemon_proof")
                == "FIELDMESH_IIO_BURST_NATIVE_TRANSPORT_BACKGROUND_DAEMON v1"
                and result.get("transport_background_daemon_start_proof")
                == "FIELDMESH_IIO_BURST_NATIVE_TRANSPORT_BACKGROUND_DAEMON v1"
                and result.get("transport_background_daemon_status_proof")
                == "FIELDMESH_IIO_BURST_NATIVE_TRANSPORT_BACKGROUND_DAEMON v1"
                and result.get("transport_background_daemon_status_started") is True
                and result.get("transport_background_daemon_xfer") is True
                and isinstance(result.get("transport_background_daemon_start_count"), int)
                and result.get("transport_background_daemon_start_count") >= 1
                and isinstance(result.get("transport_background_daemon_xfer_count"), int)
                and result.get("transport_background_daemon_xfer_count") >= 1
                and isinstance(
                    result.get("transport_background_daemon_scheduled_request_count"), int
                )
                and result.get("transport_background_daemon_scheduled_request_count") >= 1
                and result.get("python_direct_service_loop_run") is False
                and result.get("python_scheduler_drain_submission") is False
                and result.get("python_autonomous_loop_run_submission") is False
                and result.get("next_boundary") == "native_transport_worker_autonomous_daemon"
                for result in command_results
            )
        )
    ...
    native_iio_burst_integrated_rf_service_daemon_proven = bool(
        ...
        or (
            ...
            and any(
                result.get("name") == "iio_burst_helper"
                and result.get("returncode") == 0
                and result.get("native_iio_burst_integrated_rf_service_daemon") is True
                and result.get("native_iio_burst_integrated_rf_service_daemon_proof")
                == "FIELDMESH_IIO_BURST_INTEGRATED_RF_SERVICE_DAEMON v1"
                and result.get("transport_integrated_rf_service_daemon_start_proof")
                == "FIELDMESH_IIO_BURST_INTEGRATED_RF_SERVICE_DAEMON v1"
                and result.get("transport_integrated_rf_service_daemon_enqueue_proof")
                == "FIELDMESH_IIO_BURST_INTEGRATED_RF_SERVICE_DAEMON v1"
                and result.get("transport_integrated_rf_service_daemon_status_proof")
                == "FIELDMESH_IIO_BURST_INTEGRATED_RF_SERVICE_DAEMON v1"
                and result.get("transport_integrated_rf_service_daemon_status_started")
                is True
                and isinstance(
                    result.get("transport_integrated_rf_service_daemon_start_count"), int
                )
                and result.get("transport_integrated_rf_service_daemon_start_count") >= 1
                and isinstance(
                    result.get("transport_integrated_rf_service_daemon_enqueue_count"), int
                )
                and result.get("transport_integrated_rf_service_daemon_enqueue_count") >= 1
                and isinstance(
                    result.get("transport_integrated_rf_service_daemon_drained_count"), int
                )
                and result.get("transport_integrated_rf_service_daemon_drained_count") >= 1
                and result.get("python_background_daemon_start_submission") is False
                and result.get("python_autonomous_loop_run_submission") is False
                for result in command_results
            )
        )
    ...
    native_iio_burst_state_daemon_modem_profile_proven = bool(
        ...
        or (
            ...
            and any(
                ...
                and result.get("native_iio_burst_state_daemon_transport_modem_profile")
                is True
                and result.get(
                    "native_iio_burst_state_daemon_transport_modem_profile_proof"
                )
                == "FIELDMESH_IIO_BURST_STATE_DAEMON_TRANSPORT_MODEM_PROFILE v1"
                and result.get("state_daemon_transport_modem_profile_request") is True
                and result.get("iio_helper_consumes_selected_modem_profile") is True
                and result.get("state_daemon_transport_selected_samples_per_symbol")
                == plan["iq_burst"]["samples_per_symbol"]
                and result.get("state_daemon_transport_selected_bit_repeat")
                == plan["iq_burst"]["bit_repeat"]
                and result.get("python_iio_helper_modem_profile_mapping") is False
                and result.get("python_selected_modem_profile_fields") is False
                for result in command_results
            )
        )
    )
