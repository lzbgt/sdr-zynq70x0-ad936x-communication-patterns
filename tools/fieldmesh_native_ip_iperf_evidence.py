...
def _validate_iio_ack_pipeline(report: dict[str, Any], label: str) -> list[str]:
    ...
    if report.get("iio_bridge_fast_primary_min_raw_bitrate_bps") != 300_000:
        errors.append(f"{label}: fast-primary PHY floor must be 300 kbps")
    ...
    elif float(min_phy_raw) < 300_000.0:
        errors.append(
            f"{label}: IIO bridge minimum PHY raw bitrate must be at least 300 kbps"
        )
    ...
    elif float(min_primary_phy_raw) < 300_000.0:
        errors.append(
            f"{label}: IIO bridge minimum primary PHY raw bitrate must be at least 300 kbps"
        )
    ...
    elif float(min_effective_phy_raw) < 300_000.0:
        errors.append(
            f"{label}: IIO bridge minimum effective PHY raw bitrate must be at least 300 kbps"
        )
