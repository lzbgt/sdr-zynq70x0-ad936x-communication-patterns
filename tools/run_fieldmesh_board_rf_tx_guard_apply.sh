...
remote_action_policy_self_test="/tmp/fieldmesh_rf_guard_action_policy_self_test.ndjson"
...
sshpass -p "$ssh_pass" ssh "${ssh_args[@]}" "$remote" \
  "uname -a; command -v fieldmesh-udp-probe || true; \
   fieldmesh-udp-probe --help 2>&1 | grep -q rf-guard-apply && echo rf_guard_apply=present || echo rf_guard_apply=missing; \
...
if [[ "$force_upload" == "1" ]] ||
   ! grep -q '^rf_guard_apply=present$' "$out_dir/board_probe.txt" ||
   ! grep -q '^rf_guard_action_policy_self_test=present$' "$out_dir/board_probe.txt"; then
  if [[ "$upload_if_missing" != "1" ]]; then
    echo "Board fieldmesh-udp-probe lacks current rf-guard apply/action-policy self-test contract and UPLOAD_IF_MISSING=0" >&2
    exit 1
  fi
