...
work_root="${WORK_ROOT:-$repo_root/.config/fieldmesh/control-overlay-$variant}"
...
"$repo_root/tools/fieldmesh_vivado_overlay_patch.py" \
  ...
  --control-overlay \
  --apply >"$work_root/fieldmesh_control_overlay_patch.json"

cat >"$work_project/fieldmesh_control_overlay_check.tcl" <<TCL
...
if {[llength [get_bd_pins -quiet fieldmesh_ctrl/irq]] != 1} {
  error "fieldmesh_ctrl/irq pin missing"
}
...
validate_bd_design
puts "FIELDMESH_CONTROL_OVERLAY_CHECK_PASS variant=$variant part=$part"
TCL
...
cd "$work_project"
vivado -mode batch -source fieldmesh_control_overlay_check.tcl -notrace | tee "$work_root/fieldmesh_control_overlay_check.log"
rg 'FIELDMESH_CONTROL_OVERLAY_CHECK_PASS' "$work_root/fieldmesh_control_overlay_check.log" >/dev/null

printf 'fieldmesh_control_overlay_check=%s\n' "$variant"
printf 'work_root=%s\n' "$work_root"
