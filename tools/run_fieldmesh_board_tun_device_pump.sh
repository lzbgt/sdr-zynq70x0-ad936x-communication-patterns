...
out_dir="${OUT_DIR:-$repo_root/.config/fieldmesh/board-tun-device-pump-$(date +%Y%m%d-%H%M%S)}"
...
python3 - "$board_ip" "$port" "$timeout_ms" > "$out_dir/host_live_request.ndjson" <<'PY' &
import socket
import sys
...
sock.sendto(b"FIELDMESH_TUN_DEV_PUMP v1 ALLOW_LIVE_TUN_READ", (host, port))
...
set +e
sshpass -p "$ssh_pass" ssh "${ssh_args[@]}" "$remote" \
    "{ ip link set dev swarm0 up; ip route replace 10.77.2.0/24 dev swarm0; ip -json addr show dev swarm0; ping -c 1 -W 1 10.77.2.20; } > '$remote_inject' 2>&1"
inject_rc=$?
wait "$query_pid"
query_rc=$?
set -e
...
python3 - "$out_dir" "$board_ip" "$variant" "$inject_rc" <<'PY'
import json
import sys
...
live = [row for row in rows if row.get("event") == "sdk_daemon_tun_device_pump_live"]
if not live:
    raise SystemExit("missing live TUN device pump response")
event = live[0]
if event.get("ok") != 1:
