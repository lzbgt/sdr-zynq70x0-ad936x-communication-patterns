...
hello_wire = next((line for line in query_path.read_text(encoding="utf-8").splitlines()
...
if not end or end[-1].get("handled") != 50:
...
if len(hello_wire.encode("utf-8")) > 1400:
    raise SystemExit("board SDK daemon HELLO response must stay below 1400 bytes")
...
if hello[0].get("production_ready") != 0:
    raise SystemExit("board SDK daemon HELLO must not claim production readiness before RF PHY verification")
if hello[0].get("production_readiness") != "infrastructure_verified_rf_phy_pending":
    raise SystemExit("board SDK daemon HELLO production readiness state changed")
if hello[0].get("planned_features_production_level") != 0:
    raise SystemExit("board SDK daemon HELLO must not mark planned features production-level")
if hello[0].get("app_verified_real_rf") != 0 or hello[0].get("rf_phy_tx_rx_verified") != 0:
    raise SystemExit("board SDK daemon HELLO must not claim real RF app/PHY verification")
if hello[0].get("production_blocker") != "real_rf_phy_tx_rx_not_verified":
    raise SystemExit("board SDK daemon HELLO production blocker changed")
...
for key in ("firmware_ring_tx_queued", "firmware_ring_tx_owned_by_pl",
    ...
    if tun_service_status[0].get(key) != 0:
        raise SystemExit(f"board SDK daemon guarded TUN service status {key} must be 0")
if not tun_service_irq_mask_guard:
    raise SystemExit("board SDK daemon firmware ring IRQ mask guard missing")
if tun_service_irq_mask_guard[0].get("ok") is not False:
    raise SystemExit("board SDK daemon firmware ring IRQ mask must be guarded")
if tun_service_irq_mask_guard[0].get("error") != "firmware_ring_irq_mask_requires_guard":
    raise SystemExit("board SDK daemon firmware ring IRQ mask guard changed")
if tun_service_irq_mask_guard[0].get("requires_allow_firmware_ring_writes") != 1:
    raise SystemExit("board SDK daemon firmware ring IRQ mask must require write opt-in")
if not tun_service_irq_ack_guard:
    raise SystemExit("board SDK daemon firmware ring IRQ ACK guard missing")
if tun_service_irq_ack_guard[0].get("ok") is not False:
    raise SystemExit("board SDK daemon firmware ring IRQ ACK must be guarded")
if tun_service_irq_ack_guard[0].get("error") != "firmware_ring_irq_ack_requires_guard":
    raise SystemExit("board SDK daemon firmware ring IRQ ACK guard changed")
if tun_service_irq_ack_guard[0].get("requires_allow_firmware_ring_writes") != 1:
    raise SystemExit("board SDK daemon firmware ring IRQ ACK must require write opt-in")
if not tun_service_irq_wait:
    raise SystemExit("board SDK daemon firmware ring IRQ wait response missing")
if tun_service_irq_wait[0].get("ok") is not False:
    raise SystemExit("board SDK daemon firmware ring IRQ wait must not pass without mapped ring")
if tun_service_irq_wait[0].get("error") != "firmware_ring_not_mapped":
    raise SystemExit("board SDK daemon firmware ring IRQ wait guard changed")
if tun_service_irq_wait[0].get("requested_bits") != 3:
    raise SystemExit("board SDK daemon firmware ring IRQ wait requested bits changed")
for row_name, row in (("mask", tun_service_irq_mask_guard[0]),
                      ("ack", tun_service_irq_ack_guard[0]),
                      ("wait", tun_service_irq_wait[0])):
    for key in ("writes_hardware", "uses_iio", "uses_json_on_air"):
        if row.get(key) != 0:
            raise SystemExit(f"board SDK daemon firmware IRQ {row_name} key {key} must be 0")
    if row.get("hot_path_language") != "c":
        raise SystemExit(f"board SDK daemon firmware IRQ {row_name} must keep C hot path")
...
if not rf_worker_start or rf_worker_start[0].get("error") != "tun_service_not_running":
    raise SystemExit("board SDK daemon RF worker start guard failed")
if not rf_worker_status or rf_worker_status[0].get("driver_queue_worker") != 1:
    raise SystemExit("board SDK daemon RF worker status response failed")
if rf_worker_status[0].get("running") != 0 or rf_worker_status[0].get("rf_phy_tx_rx") != 0:
    raise SystemExit("board SDK daemon RF worker status must remain RF-PHY-pending")
if not rf_worker_phy_plan or rf_worker_phy_plan[0].get("ok") is not True:
    raise SystemExit("board SDK daemon RF worker PHY plan response failed")
for key in ("requires_sidecar_preflight", "requires_sidecar_dma",
    ...
    if rf_worker_phy_plan[0].get(key) != 1:
        raise SystemExit(f"board SDK daemon RF worker PHY plan key {key} must be 1")
for key in ("sidecar_preflight_passed", "sidecar_dma_passed",
    ...
    if rf_worker_phy_plan[0].get(key) != 0:
        raise SystemExit(f"board SDK daemon RF worker PHY plan key {key} must be 0")
if rf_worker_phy_plan[0].get("next_boundary") != "rf_phy_driver_tx_rx":
    raise SystemExit("board SDK daemon RF worker PHY plan next boundary changed")
if rf_worker_phy_plan[0].get("production_blocker") != "rf_dac_source_select_not_verified":
    raise SystemExit("board SDK daemon RF worker PHY plan production blocker changed")
if not rf_phy_bind_validate or rf_phy_bind_validate[0].get("ok") is not True:
    raise SystemExit("board SDK daemon RF PHY driver bind validation response failed")
if rf_phy_bind_validate[0].get("driver") != "fieldmesh_rf_packet_engine":
    raise SystemExit("board SDK daemon RF PHY bind validation driver changed")
if rf_phy_bind_validate[0].get("adapter_name") != "swarm0":
    raise SystemExit("board SDK daemon RF PHY bind validation adapter changed")
for key in ("requires_sidecar_preflight", "requires_sidecar_dma",
            "requires_rf_packet_engine", "requires_rf_tx_guard",
            "requires_rf_dac_source_select",
            "requires_conducted_or_shielded", "requires_legal_frequency_profile",
    ...
    if rf_phy_bind_validate[0].get(key) != 1:
        raise SystemExit(f"board SDK daemon RF PHY bind validation key {key} must be 1")
for key in ("conducted_or_shielded", "legal_frequency_profile", "rx_first",
    ...
    if rf_phy_bind_validate[0].get(key) != 0:
        raise SystemExit(f"board SDK daemon RF PHY bind validation key {key} must be 0")
if rf_phy_bind_validate[0].get("sidecar_preflight_passed") != 1:
    raise SystemExit("board SDK daemon RF PHY bind validation sidecar preflight flag changed")
if rf_phy_bind_validate[0].get("sidecar_dma_passed") != 1:
    raise SystemExit("board SDK daemon RF PHY bind validation sidecar DMA flag changed")
...
if rf_phy_bind_validate[0].get("rf_dac_source_select_passed") != 0:
    raise SystemExit("board SDK daemon RF PHY bind validation must not fake DAC source-select evidence")
if rf_phy_bind_validate[0].get("driver_prerequisites_ready") != 0:
    raise SystemExit("board SDK daemon RF PHY bind validation must require DAC source-select evidence")
...
if rf_phy_bind_validate[0].get("production_blocker") != "rf_dac_source_select_not_verified":
    raise SystemExit("board SDK daemon RF PHY bind validation production blocker changed")
if not rf_phy_bind_apply or rf_phy_bind_apply[0].get("ok") is not False:
    raise SystemExit("board SDK daemon RF PHY bind apply guard failed")
if rf_phy_bind_apply[0].get("error") != "live_rf_phy_not_authorized":
    raise SystemExit("board SDK daemon RF PHY bind apply error changed")
if rf_phy_bind_apply[0].get("requires_rf_dac_source_select") != 1:
    raise SystemExit("board SDK daemon RF PHY bind apply must require DAC source-select evidence")
for key in ("live_rf_allowed", "rf_phy_tx_rx", "rf_phy_tx_rx_verified",
            "app_verified_real_rf", "production_ready", "opens_iio_buffers",
            "starts_rf_tx", "writes_hardware", "commands_executed",
            "uses_inter_board_ip_routing", "uses_json_on_air"):
    if rf_phy_bind_apply[0].get(key) != 0:
        raise SystemExit(f"board SDK daemon RF PHY bind apply key {key} must be 0")
if not rf_worker_stop or rf_worker_stop[0].get("ok") is not True:
    raise SystemExit("board SDK daemon RF worker stop response failed")
...
print(json.dumps({
    ...
    "rf_worker_phy_plan_events": len(rf_worker_phy_plan),
    "rf_phy_bind_validate_events": len(rf_phy_bind_validate),
    "rf_phy_bind_apply_events": len(rf_phy_bind_apply),
