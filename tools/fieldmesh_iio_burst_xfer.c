                      ...
                      const struct options *base)
{
    ...
    while (fgets(line, sizeof(line), stdin)) {
        ...
        if (!worker_xfer && !service_loop_xfer && !scheduler_drain && !legacy_xfer) {
            printf("{\"event\":\"fieldmesh_iio_burst_xfer\",\"ok\":false,"
                   "\"error\":\"expected_TRANSPORT_WORKER_or_SERVICE_LOOP_or_SCHEDULER_or_WORKER_XFER_or_XFER_or_QUIT\"}\n");
            fflush(stdout);
            continue;
        }
        if ((worker_xfer || service_loop_xfer || scheduler_drain) && !transport_session_started) {
            printf("{\"event\":\"fieldmesh_iio_burst_xfer\",\"ok\":false,"
                   "\"native_iio_burst_transport_worker\":true,"
                   "\"native_iio_burst_transport_worker_proof\":\"FIELDMESH_IIO_BURST_NATIVE_TRANSPORT_WORKER v1\","
                   "\"native_iio_burst_transport_session\":false,"
        ...
        if ((service_loop_xfer || scheduler_drain) && !transport_service_loop_started) {
            printf("{\"event\":\"fieldmesh_iio_burst_xfer\",\"ok\":false,"
                   "\"native_iio_burst_transport_worker\":true,"
                   "\"native_iio_burst_transport_worker_proof\":\"FIELDMESH_IIO_BURST_NATIVE_TRANSPORT_WORKER v1\","
                   "\"native_iio_burst_transport_session\":true,"
        ...
        if (scheduler_drain && !transport_scheduler_started) {
            printf("{\"event\":\"fieldmesh_iio_burst_xfer\",\"ok\":false,"
                   "\"native_iio_burst_transport_worker\":true,"
                   "\"native_iio_burst_transport_service_loop\":true,"
                   "\"native_iio_burst_transport_scheduler\":false,"
        ...
        req.native_transport_worker_mode = worker_xfer || service_loop_xfer || scheduler_drain;
        req.native_transport_session_mode = transport_session_started;
        req.native_transport_service_loop_mode = service_loop_xfer || scheduler_drain;
        req.native_transport_scheduler_mode = scheduler_drain;
        req.transport_session_start_count = transport_session_start_count;
        req.transport_worker_request_count = (worker_xfer || service_loop_xfer || scheduler_drain) ?
        ...
        req.transport_scheduler_drain_count = scheduler_drain ?
            transport_scheduler_drain_count + 1ULL : 0ULL;
        req.transport_scheduler_scheduled_request_count = scheduler_drain ?
        ...
        if (worker_xfer || service_loop_xfer || scheduler_drain) {
            ...
            char *request_line = worker_xfer ? line + 12 :
                (service_loop_xfer ? line + 27 : line + 26);
            for (char *token = strtok_r(request_line, " ", &save);
                 token;
                 token = strtok_r(NULL, " ", &save)) {
                const char *value = value_after(token, scheduler_drain ?
                                                "queue_file=" : "request_file=");
                if (value) {
                    request_file = value;
                }
            }
            if (scheduler_drain && request_file &&
            ...
            if (!request_file || load_worker_request_file(&req, request_file) != 0) {
                ...
                printf("{\"event\":\"fieldmesh_iio_burst_xfer\",\"ok\":false,"
                       ...
                       (service_loop_xfer || scheduler_drain) ? "true" : "false",
                       scheduler_drain ? "true" : "false");
                fflush(stdout);
                continue;
            }
        } else {
        ...
        if (service_loop_xfer || scheduler_drain) {
            transport_service_loop_run_count++;
        }
        if (scheduler_drain) {
            transport_scheduler_drain_count++;
