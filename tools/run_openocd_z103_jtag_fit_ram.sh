...
fit_image="${FIT_IMAGE:-$z103_source/build/pluto.itb}"
...
uboot_command_interval_seconds="${UBOOT_COMMAND_INTERVAL_SECONDS:-0.5}"
bootargs="${BOOTARGS:-console=ttyPS0,115200 maxcpus=1 rootfstype=ramfs root=/dev/ram0 rw earlyprintk clk_ignore_unused uboot=jtag-fit-ram}"

for path in "$ps7_init" "$uboot_elf" "$fit_image"; do
  if [[ ! -f "$path" ]]; then
...
if [[ -n "$capture" ]]; then
  ...
  : >"$capture"
  {
    echo "# OpenOCD Z103 JTAG FIT RAM boot"
    echo "# PS7 init: $ps7_init"
    echo "# U-Boot ELF: $uboot_elf"
    echo "# FIT image: $fit_image -> $fit_addr"
...
echo LOAD_FIT_IMAGE
load_image {$fit_image} $fit_addr bin
echo LOAD_UBOOT_ELF
...
{
  echo
  echo "# U-Boot FIT RAM boot commands"
