...
work_dir="$repo_root/.config/fieldmesh/rf-tx-guard-apply"
...
if "$probe_z203" rf-guard-apply \
  ...
  --slot-index 3 \
  >/dev/null 2>&1; then
  echo "rf-guard-apply accepted missing --allow-live-writes" >&2
  exit 1
fi
...
if "$probe_z203" rf-guard-apply \
  ...
  --slot-index 3 \
  >/dev/null 2>&1; then
  echo "rf-guard-apply accepted missing --target-is-zynq-board" >&2
  exit 1
fi

"$probe_z203" rf-guard-apply \
  --ctrl-mem-file "$mem_file" \
  --preflight-assert "$preflight" \
  --allow-live-writes \
  --conducted-or-shielded \
  ...
  --slot-index 3 \
  --arm-window-us 5000 \
  >"$work_dir/rf_guard_apply.ndjson"
...
apply = rows("rf_guard_apply.ndjson")
