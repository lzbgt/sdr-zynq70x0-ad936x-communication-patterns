...
| Native TCP/IP client mode | Implemented through `swarm0`, daemon service, BLR `APP_DATA`, daemon RF worker lifecycle, RF TX lease/ACK, and RX ingest | ICMP plus TCP/UDP socket gates over the daemon RF-worker bridge | Connect daemon RF worker to real RF PHY TX/RX, then measure throughput, RTT, loss, retransmits, and failover |
