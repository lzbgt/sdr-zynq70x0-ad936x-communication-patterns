...
- live over-air board-to-board `iperf3` has a separate bridge mechanism:
  ...
  BPSK 8-sample/repeat-1 in both directions at 3.072 Msps, raising the raw modem
  PHY ceiling to 384 kbit/s while keeping 32-sample/repeat-2 as the stronger
  retry profile. Production evidence requires at least 300 kbit/s minimum raw
  ...
  no retry use, and effective raw rate at or above 300 kbit/s; `retry_fallback`
  ...
  `samples_per_symbol=8`, `bit_repeat=1` at the 3.072 Msps native-IP IIO
  sample rate, lifting the raw modem PHY ceiling to 384 kbit/s before MAC/IP
