...
emission, header CRC-16 rejection, truncated-burst drops, recovery on the next
valid burst, resync, bad-class drops, and fault accounting.
...
and, for the RF-engine overlay, the extended `0x140..0x23c` sidecar range that
also carries QPSK RX demod quality, PL phase-weighted matched-filter timing recovery,
DC-offset and carrier phase correction, and
acquisition/framing diagnostics. The shared descriptor
...
passing on reports collected through per-burst helper startup. It also requires
...
sends one C-owned state-daemon transport lifecycle request with
`TRANSPORT_STATE_DAEMON_LIFECYCLE_XFER_FIELDS`; the helper starts the native
transport layers, queues native transfer fields, and waits for the completed
transfer report inside C. Accepted reports must include
...
`python_background_daemon_start_submission=false`, and
`python_transport_request_file_submission=false`, and
`python_transport_scheduler_queue_file_submission=false`, and
`python_transport_helper_command_status_pacing=false`, and
...
`FIELDMESH_RF_SERVICE_TRANSPORT_LOOP_TICK v1`, which queries peer scheduler
status from C and then enters `FIELDMESH_RF_SERVICE_LOOP_TICK v1` for C-owned
service-burst emission in one native transport tick. They now also require
`FIELDMESH_RF_SERVICE_LOOP_START/STATUS v1`, proving a persistent native
service-loop worker session tracked tick, burst, skip, preemption, and
multiplexing counters around those ticks. The same
worker/scheduler/decision/transport-loop/tick/loop-worker proof is preserved
through native-IP evidence, app reports, and readiness.
...
C TX-backend readback proof plus hash-verified evidence manifest. Missing
reports, preflight-only reports, blocked sub-gates, stale TX-backend evidence,
or an unverified RF archive keep `production_ready=false` and surface their
blockers in one JSON object.
...
sequence, its `tx_backend_readback_report`, and its
`evidence_manifest_sha256`. `REAL_RF_PRODUCTION_GATE_REPORT` can still be
supplied as supporting gate detail.
...
After correcting `tools/build_fieldmesh_rf_engine_overlay_vivado.sh` so Z103
defaults to the RF-engine overlay instead of the control/ring overlay, the lean
Z103 QPSK RF-engine Vivado build completed timing-clean. The canonical Z103
runtime package was regenerated from that RF-engine bitstream and the matching
...
The routed timing report for the corrected Z103 RF-engine build reported all
constraints met, with `clk_fpga_0` WNS `+3.408 ns` and `rx_clk` WNS `+8.143 ns`.
...
Z103 pluto.frm:      4344ec56ce45607846a06582180e58ebc7e59580907be7e78cd38f3162ed64ca
Z103 pluto.itb:      0ea0fb1f7f2fdd39d8142e06ae0fde02d2ec029e25a7784af8b08529a7cfbd1f
