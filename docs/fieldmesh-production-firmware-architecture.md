...
regression reports, HIL/test glue, and factory diagnostics. In particular,
`fieldmesh_iio_rf_worker_bridge_loop.py` must never be treated as a
performance-critical packet/RF pipeline; production readiness evidence must
mark that script as test glue only and bind the performance-critical owner to
C firmware and FPGA logic.
...
descriptor-validated egress bytes are broadcast to RX DMA and the PL-whitened,
pulse-shaped QPSK TX chain (`fieldmesh_qpsk_tx_whitener`,
`fieldmesh_qpsk_symbolizer`, and `fieldmesh_qpsk_tx_fir`), while firmware-DMA
controls still reset off and require explicit software arming before
RF-symbolizer ingress can move. The
...
sidecar window. `0x140..0x23c` contains fixed binary firmware-DMA control and
...
builds extend the same sidecar aperture with QPSK RX demod quality, PL RX FIR
...
For `ACTION=latency-budget`, it requires the C-decoded
`latency_budget_allowed=true` policy before it writes the FPGA service-latency
budget register.
