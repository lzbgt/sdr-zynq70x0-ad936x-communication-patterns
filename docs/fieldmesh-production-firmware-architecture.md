...
descriptor gate for this policy, so the later BRAM/DMA MAC path can share the
same CRC and descriptor-local semantic checks instead of reimplementing them in
the AXI-lite shell. Rejected service attempts clear the slot's RX descriptor,
