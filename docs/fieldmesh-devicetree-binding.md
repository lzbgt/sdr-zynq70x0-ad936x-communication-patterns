...
The generator renders the sidecar nodes from
`sdk/c/include/fieldmesh_sidecar_addr.h`, the same C address contract consumed
by native board tools for their default scan and control addresses.
`fieldmesh-udp-probe sidecar-addr-self-test` reports the compiled C view of
this table without touching `/dev/mem`.
