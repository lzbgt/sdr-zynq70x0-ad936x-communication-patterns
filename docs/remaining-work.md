...
remain `production_ready=0`. Those app feature reports must reference the same
bridge and IQ live-run evidence so a bridge decode from one run cannot be paired
with app behavior from another.
