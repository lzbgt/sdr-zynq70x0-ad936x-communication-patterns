...
the first autonomous full-MTU BRAM firmware-ring boundary.
`fieldmesh_firmware_mac_scheduler` and
...
evidence cannot be mistaken for the production RF/IP data plane. The native-IP
...
counters plus RF burst total/live-run/decode timing, so captures can prove
whether the bounded pipeline was actually exercised and separate ACK latency
from RF burst cost. With the previous
...
both directions at 15.36 Msps with a 10 MHz RF bandwidth default, raising the raw
...
The bridge now takes a first step past whole-batch service as well: native-IP
HIL defaults daemon RF leases to four frames while limiting each encoded RF
...
Those scheduler assumptions now have a native C contract:
...
consumes it. Production service now calls
`FIELDMESH_RF_SERVICE_TRANSPORT_LOOP_TICK v1`; the source daemon queries its peer
scheduler status in C and enters `FIELDMESH_RF_SERVICE_LOOP_TICK v1` with a
native cross-daemon transport-loop proof. The bridge also starts and audits
`FIELDMESH_RF_SERVICE_LOOP_START/STATUS v1`, so native-IP evidence proves a
persistent native bidirectional service-loop worker session owned the
tick/burst/skip/preemption/multiplexing counters. The Python outer loop still
launches RF/IIO burst work, but per-tick peer-score collection is now inside the
daemon transport boundary. The next step is moving the burst transport worker
itself into native code rather than having Python invoke each RF burst.
...
FPGA MAC-service latency cycles plus the DMA TX-completion poll count as
latency evidence and now requires firmware-DMA progression across the
smoke/bind interval: TX parser packet/byte,
...
- Consolidate the reviewed `design.md` production insight into implementation:
  ...
  acquisition preamble and maps packet bytes into repeated signed QPSK I/Q
  symbols while keeping tuning, filtering, TX enable, and scheduled launch
...
- Bind the same shim frame to an integrated PL/driver loopback endpoint while
  ...
  the UIO firmware-ring probe, the devicetree planner, the Vivado DMA
  inventory, and the overlay patcher share one address contract; the inventory
  helper's `--check-sidecar` mode now fails if those windows collide with
  imported Vivado Tcl.
  `tools/fieldmesh_sidecar_plan.py` now emits the checked sidecar plan in JSON,
  Markdown, or Tcl constants form for the later Vivado overlay step, and its
  `--check-rtl` mode verifies the
  required RTL
  ...
  rotation, acquisition/search counters, demod symbol quality/margin, PL
  DC-offset correction estimates, stream backpressure counters, RX
  packet/drop/CRC counters, resyncs, and framer fault status for live
  acquisition bottleneck triage. The C SDK header
  ...
  `--fw-dma-action-policy-self-test` now exercise that C status decode and
  action-policy projection path through shared C fixture vectors without
  `/dev/mem`, so CI covers active/faulted, reset-idle, and write-policy output
  without live hardware reads. The copied overlay is
...
- `tools/fieldmesh_rf_tx_enable_plan.py` now joins green sidecar preflight,
  ...
  carries the C RF guard policy proof, FieldMesh control base, preflight
  assertion, RF guard slot, frequency profile, bounded duration, fixture
  parameters, and rollback contract so the backend does not infer live-control
  policy from shell/Python state. The generated live script now delegates
  source-select, guard-arm, tune, bounded TX, TX rollback, DAC-source rollback,
  and guard rollback to the compiled backend; it no longer executes the
