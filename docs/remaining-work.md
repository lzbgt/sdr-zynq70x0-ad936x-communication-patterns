...
- Consolidate the reviewed `design.md` production insight into implementation:
  ...
  and quadrant rotation from preamble-plus-magic correlation with
  `fieldmesh_qpsk_byte_sync`, restores packet boundaries with a
...
- Bind the same shim frame to an integrated PL/driver loopback endpoint while
  ...
  `fieldmesh_ctrl` register page at `0x140..0x1b4`. The endpoint resets
