...
- Bind the same shim frame to an integrated PL/driver loopback endpoint while
  ...
  the devicetree planner, the Vivado DMA inventory, and the overlay patcher
  share one address contract; the inventory helper's `--check-sidecar` mode now
  fails if those windows collide with imported Vivado Tcl.
