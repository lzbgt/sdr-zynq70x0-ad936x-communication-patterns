...
- Use `fieldmesh_plan_rf_packet()` / `fieldmesh_submit_rf_packet()` and daemon
  ...
  and async FIFO are BD-visible behind the sidecar DMA/bridge TX path, with
  guard control/status wired through the existing sidecar control window while
  the DAC-clock-domain FIFO output remains disconnected from AD936x TX.
