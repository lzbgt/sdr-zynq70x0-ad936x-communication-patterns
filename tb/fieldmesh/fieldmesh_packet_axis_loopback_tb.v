...
module fieldmesh_packet_axis_loopback_tb;
...
reg [31:0] rx_packet_addr_base = 32'd600;
wire rx_desc_valid;
...
fieldmesh_packet_axis_loopback dut (
    .clk(clk),
    .rst(rst),
    .enable(enable),
...
initial begin
    ...
    write_tx_mem(10'd40, 8'hde);
    write_tx_mem(10'd41, 8'had);
    write_tx_mem(10'd42, 8'hbe);
    write_tx_mem(10'd43, 8'hef);

    submit_tx_desc(32'd40, 16'd4, 16'd300, 8'd2, 8'd4, 16'd9);
    @(posedge clk);
    while (!rx_desc_valid) @(posedge clk);
    if (rx_desc_packet_addr != 32'd600) fail("rx packet_addr mismatch");
    if (rx_desc_packet_len != 16'd4) fail("rx packet_len mismatch");
    if (rx_desc_stream_id != 16'd300) fail("rx stream_id mismatch");
    if (rx_desc_traffic_class != 8'd2) fail("rx class mismatch");
    if (rx_desc_mode != 8'd4) fail("rx mode mismatch");
    if (rx_desc_slot != 16'd9) fail("rx slot mismatch");
    if (rx_desc_flags != FM_DESC_DONE) fail("rx flags mismatch");
    @(negedge clk);
    expect_rx_mem(10'd600, 8'hde);
    expect_rx_mem(10'd601, 8'had);
    expect_rx_mem(10'd602, 8'hbe);
    expect_rx_mem(10'd603, 8'hef);

    write_tx_mem(10'd48, 8'h11);
    write_tx_mem(10'd49, 8'h22);
    submit_tx_desc(32'd48, 16'd2, 16'd301, 8'd1, 8'd4, 16'd10);
    repeat (3) @(negedge clk);
    if (sink_byte_count != 32'd4) fail("sink advanced while rx descriptor pending");
    ...
    @(posedge clk);
    while (!rx_desc_valid) @(posedge clk);
    if (rx_desc_packet_addr != 32'd600) fail("second rx packet_addr mismatch");
    if (rx_desc_packet_len != 16'd2) fail("second rx packet_len mismatch");
    if (rx_desc_stream_id != 16'd301) fail("second rx stream_id mismatch");
    if (rx_desc_traffic_class != 8'd1) fail("second rx class mismatch");
    @(negedge clk);
    expect_rx_mem(10'd600, 8'h11);
    expect_rx_mem(10'd601, 8'h22);

    if (source_packet_count != 32'd2) fail("source packet_count mismatch");
    if (source_byte_count != 32'd6) fail("source byte_count mismatch");
    if (sink_packet_count != 32'd2) fail("sink packet_count mismatch");
    if (sink_byte_count != 32'd6) fail("sink byte_count mismatch");
    if (source_drop_count != 32'd0) fail("source drop_count mismatch");
    if (sink_drop_count != 32'd0) fail("sink drop_count mismatch");
    if (source_fault || sink_fault) fail("unexpected stream fault");

    $display("PASS: fieldmesh_packet_axis_loopback_tb");
    $finish;
end
