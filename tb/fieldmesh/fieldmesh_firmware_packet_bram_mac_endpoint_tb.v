...
module fieldmesh_firmware_packet_bram_mac_endpoint_tb;
...
task wait_pump_done;
    ...
    begin
        ...
        if (!pump_done) fail("MAC endpoint pump did not finish");
...
initial begin
    ...
    pulse_tick();
    if (!mac_scheduler_active || mac_empty_tick_count != 32'd1 ||
        mac_pump_start_count != 32'd0) begin
        fail("MAC endpoint empty tick mismatch");
    end
    ...
    load_desc(16'd0, 8'd3, 32'h0000_0600, 32'd0);
    load_desc(16'd1, 8'd1, 32'h0000_0601, PACKET_STRIDE);
    #1;
    if (service_queued_count != 16'd2) fail("MAC endpoint queued count mismatch");
    if (service_selected_word != 32'h8001_0001) fail("MAC endpoint selection mismatch");

    mac_service_budget = 16'd4;
    pulse_tick();
    wait_pump_done();
    if (!pump_drained_empty || pump_budget_exhausted || pump_error_seen) begin
        fail("MAC endpoint pump flags mismatch");
    end
    if (mac_pump_start_count != 32'd1 || mac_pump_done_count != 32'd1 ||
        mac_drained_empty_count != 32'd1 || mac_budget_exhausted_count != 32'd0) begin
        fail("MAC endpoint scheduler counters mismatch");
    end
    if (pump_services_started != 32'd2 || pump_services_completed != 32'd2 ||
        pump_accepted_count != 32'd2 || desc_publish_count != 32'd2 ||
        bram_service_count != 32'd2) begin
        fail("MAC endpoint pump/endpoint counters mismatch");
    end
    ...
    desc_expect_word(REGION_ACK, 16'd0, 16'd1, 32'h0000_0600);
    desc_expect_word(REGION_ACK, 16'd1, 16'd1, 32'h0000_0601);
    ...
    pulse_tick();
    if (mac_empty_tick_count != 32'd2 || mac_pump_start_count != 32'd1) begin
        fail("MAC endpoint post-drain empty tick mismatch");
    end

    if (mac_tick_count != 32'd3) fail("MAC endpoint tick count mismatch");
    if (mac_busy_tick_count != 32'd0 || mac_error_seen_count != 32'd0 ||
        mac_stop_count != 32'd0) begin
        fail("MAC endpoint unexpected scheduler fault counters");
    end
    if (desc_bounds_error_count != 32'd0 || packet_bounds_error_count != 32'd0 ||
        ...
        fail("MAC endpoint unexpected service faults");
    end

    $display("PASS: fieldmesh_firmware_packet_bram_mac_endpoint_tb");
    $finish;
end
