...
- Live AD936x HIL now proves the software buffer path is material, not just a
  ...
  persistent helper now runs one C-owned state-daemon transport lifecycle command
  through `TRANSPORT_STATE_DAEMON_LIFECYCLE_XFER_FIELDS`, which starts the
  native transport layers, queues transfer fields, waits for completion in C, and
  reports
  ...
  effective PHY rate and cannot satisfy the high-rate gate. The RF service
  ...
  itself to report positive libiio execution counts. The Python bridge and the
