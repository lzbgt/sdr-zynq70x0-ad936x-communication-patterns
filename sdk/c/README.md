...
- `examples/fieldmesh_firmware_uio_ring_probe.c` binds the same linear layout
  ...
  `fieldmesh_firmware_ring_desc_store` keeps TX/RX/ACK descriptor RAM separate
  from AXI-lite register handling and gives the future endpoint reusable publish,
  clear, and TX-done semantics.
  ...
  `fieldmesh_firmware_axis_bram_mac_endpoint` composes both the ingress writer
  ...
  `fieldmesh_ctrl` registers at `0x140..0x210`; reset leaves the endpoint
  ...
  sync input/output bytes, demod symbol quality/margin, PL DC-offset correction
  estimates, stream backpressure counters, lock/slip/rotation/search-drop counts, RX
  ...
  `--fw-dma-action-policy-self-test` decode fixed C register vectors from
  `fieldmesh_fw_dma_status_test_regs_active_faulted()` and
  `fieldmesh_fw_dma_status_test_regs_idle()` without `/dev/mem`, so CI covers
  active/faulted, reset-idle, and write-policy projections without hardware
  ...
  `fieldmesh_fw_ring_irq_mask_write()`. The daemon exposes this hardware
  interrupt control through guarded
...
- `include/fieldmesh_rf_service_policy.h` is the C contract for the current
  ...
  `FIELDMESH_RF_SERVICE_TRANSPORT_LOOP_TICK v1` to query peer scheduler status
  from C before entering `FIELDMESH_RF_SERVICE_LOOP_TICK v1` for C-owned burst
  emission. `FIELDMESH_RF_SERVICE_LOOP_START/STATUS v1` wrap those ticks in a
  persistent native service-loop worker session with
  ...
  slots before emitting the sub-burst. Python still launches the RF/IIO burst
  work, but per-tick cross-daemon peer-score collection now lives behind the C
  transport-loop command.
